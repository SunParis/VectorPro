...
time_struct *build_test_1(size_t size, bool first_flag) {
    ...
    if (first_flag == true) {
        ...
        vector_pro<small_object> test2(size);
...
time_struct *build_test_3(size_t size, bool first_flag) {
    ...
    if (first_flag == true) {
        ...
        ret[PRO].start = get_tick_count();
        vector_pro<large_object> test2(size);
        ret[PRO].build = get_tick_count();
        //
        ret[PRO].end = get_tick_count();
        test2.clear();
...
int main() {
    ...
    time_struct *res;
    fstream output_classic_build("./tests/results/build_test_classic.txt", ofstream::out);
    fstream output_pro_build("./tests/results/build_test_pro.txt", ofstream::out);
